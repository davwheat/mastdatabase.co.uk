...
"Cellular-Network-Info" header is only sent when the recipient device responds. The disclosure of IMEI and IMSI **still occurs** irrespective of IMS
registration status, meaning disabling 4G Calling / WiFi calling has no effect.
...
further learning into the intricacies of IMS signalling, and the way in which devices send headers, I now believe that disabling both
4G Calling and WiFi Calling **WOULD** have mitigated this attack. I believe what I originally saw may have been due to rapid testing, and
confusion between different logged events. IMEI and IMSI disclosure was still present, even when the recipient device was unreachable.
