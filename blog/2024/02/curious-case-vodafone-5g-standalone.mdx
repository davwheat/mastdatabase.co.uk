...
tests, at least until 5G moves beyond being an 'eMBB'<sup id="note-2-link">[\[1\]](#note-1)</sup> solution.

---

# Notes

<sup id="note-1">[\[1\]](#note-1-link)</sup> Dynamic spectrum sharing (DSS) allows one carrier to be used for multiple radio access technologies (RATs)
at the same time. In this case, it can be used for LTE and NR simultaneously.
